...
import { canvasToMap, generateCode, parseCode, parseJson } from "./methods";
  ...
  methods: {
    ...
    async importJson() {
      ...
      const jsonStructure = {
        ...
        npc: [
          { image: "", size: 0, label: "", systemPrompt: "", firstMessage: "" }
        ]
      };

      function validateJson(jsonData, structure) {
        if (typeof jsonData !== 'object' || jsonData === null) return false;
        return Object.keys(structure).every(key =>
          // 判断传入json是否有map和npc两类 
        jsonData.hasOwnProperty(key) &&
          Array.isArray(jsonData[key]) &&
          // 判断传入json每类中是否满足格式
          jsonData[key].every(item =>
            Object.keys(structure[key][0]).every(prop => item.hasOwnProperty(prop))
          )
        );
      }

      if (!validateJson(JSON.parse(jsonText), jsonStructure)) {
        alert("JSON结构不符合要求，请检查后重新输入。");
        return;
      }
      try {
        parseJson(jsonText);
...
<template>
  <div ref="canvas" id="gridCanvas"
    ...
    @mousedown="dragStart" @mousemove="dragging" @mouseup="(e) => {
      dragEnd();
      editEnd();
    }
      " @mouseleave="(e) => {
    ...
    <div v-for="i in 2500" class="border flex justify-center items-center bg-white select-none"
      :class="{ 'hover:bg-black/10': this.mode === 'PEN' }" @mousedown="editStart" @mousemove="(e) => {
        editing(e, i);
      }
        " @click="(e) => {
          clickEdit(e, i);
  ...
  <div id="exportPanel"
    ...
    <button @click="downloadCode" class="size-[32px] rounded-[8px] bg-white border flex justify-center items-center"
      title="下载代码">
      <div class="size-[20px] flex justify-center items-center">
    ...
    <button @click="uploadCode" class="size-[32px] rounded-[8px] bg-white border flex justify-center items-center"
      title="上传代码">
      <div class="size-[20px] flex justify-center items-center">
        <svg xmlns="http://www.w3.org/2000/svg" width="17.5" height="17.5" viewBox="0 0 16.67 16.32">
          <path class="cls-1"
  ...
  <div id="toolsPanel" class="absolute left-4 bottom-4 flex flex-col text-[32px] w-fit gap-2">
    ...
    <div class="flex flex-wrap size-fit gap-[8px] p-[8px] rounded-[16px] panel-background">
      ...
      <button @click="importJson
        " class="bg-[#3198FF] rounded-[8px] text-[16px] w-fit px-2 py-1 self-end text-white">
        导入JSON
      </button>
    </div>
  </div>
